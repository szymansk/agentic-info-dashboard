...
mk_repo() {
  R="$T_ROOT/repo"; rm -rf "$R" "$T_ROOT/origin.git"; mkdir -p "$R/dashboards/ai-news/archive" "$R/dashboards/it-services" "$R/docs/ai-news/archive" "$R/bin"
  ...
  # Platzhalter, damit die OWN_DIRT_PATHS-Pathspec (lib-daily.sh) beim
  # `git stash push -- dashboards/ai-news dashboards/it-services docs` immer
  # auf existierende Pfade trifft — sonst bricht stash mit "pathspec did not
  # match any files" fatal ab und die Aufräumung bleibt stecken.
  echo x > "$R/dashboards/it-services/.keep"
...
# HOME zeigt für den Lauf auf einen leeren Sandkasten-Pfad: resolve_claude()
# in lib-daily.sh prüft $HOME/.local/bin/claude VOR dem PATH — auf dieser
...
run() { local rc=0; out="$(cd "$R" && HOME="$T_ROOT/fake-home" "$R/bin/run-daily.sh" "$@" 2>&1 </dev/null)" || rc=$?; printf '%s' "$out" > "$T_ROOT/last-out"; echo "$rc"; }
